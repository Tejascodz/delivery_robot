...
set(_exported_dependencies "rclcpp;sensor_msgs;nav_msgs;geometry_msgs;visualization_msgs;tf2;tf2_ros;tf2_geometry_msgs;pcl_conversions;pcl_ros")

find_package(ament_cmake_libraries QUIET REQUIRED)

# find_package() all dependencies
# and append their DEFINITIONS INCLUDE_DIRS, LIBRARIES, and LINK_FLAGS
# variables to delivery_robot_perception_DEFINITIONS, delivery_robot_perception_INCLUDE_DIRS,
# delivery_robot_perception_LIBRARIES, and delivery_robot_perception_LINK_FLAGS.
# Additionally collect the direct dependency names in
# delivery_robot_perception_DEPENDENCIES as well as the recursive dependency names
# in delivery_robot_perception_RECURSIVE_DEPENDENCIES.
if(NOT _exported_dependencies STREQUAL "")
  find_package(ament_cmake_core QUIET REQUIRED)
  set(delivery_robot_perception_DEPENDENCIES ${_exported_dependencies})
  set(delivery_robot_perception_RECURSIVE_DEPENDENCIES ${_exported_dependencies})
  set(_libraries)
  foreach(_dep ${_exported_dependencies})
    ...
    if(NOT "${${_dep}_TARGETS}" STREQUAL "")
      foreach(_target ${${_dep}_TARGETS})
        ...
        if(TARGET "${_target}")
          ...
          if(_include_dirs)
            list_append_unique(delivery_robot_perception_INCLUDE_DIRS "${_include_dirs}")
          endif()
    ...
    if(NOT use_modern_cmake)
      if(${_dep}_DEFINITIONS)
        list_append_unique(delivery_robot_perception_DEFINITIONS "${${_dep}_DEFINITIONS}")
      endif()
      if(${_dep}_INCLUDE_DIRS)
        list_append_unique(delivery_robot_perception_INCLUDE_DIRS "${${_dep}_INCLUDE_DIRS}")
      endif()
      if(${_dep}_LIBRARIES)
        list(APPEND _libraries "${${_dep}_LIBRARIES}")
      endif()
      if(${_dep}_LINK_FLAGS)
        list_append_unique(delivery_robot_perception_LINK_FLAGS "${${_dep}_LINK_FLAGS}")
      endif()
      if(${_dep}_RECURSIVE_DEPENDENCIES)
        list_append_unique(delivery_robot_perception_RECURSIVE_DEPENDENCIES "${${_dep}_RECURSIVE_DEPENDENCIES}")
      endif()
    endif()
    if(_libraries)
      ament_libraries_deduplicate(_libraries "${_libraries}")
      list(APPEND delivery_robot_perception_LIBRARIES "${_libraries}")
    endif()
  endforeach()
endif()
